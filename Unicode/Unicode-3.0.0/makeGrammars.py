...
Usage:
    makeGrammars.py [-acefktuz] [-v] [--prefix=<PREFIX>]
...
Options:
    ...
    -p --prefix=<PREFIX>            Prefix for keywords  [default: CLASSIFY_]
...
License:GPL 3.0"""
    ...
    def g4enhance(self):
        ...
        for rule, pattern in self.combinations.iteritems():
            split1 = [pre+s.strip() for s in pattern.split('|')]

            widened = "\n       %s  // %s" % ('\n     | '.join(split1), source)
            self.g4rule(pre+rule, widened, 1)
        self.g4comment('End of Unicode codepoint classification', 1)
        #self.g4rule('WS0', '[ \t\r\n]          // hand-written rule', 1)
        pre = self.prefix
        self.g4rule(pre+'WS' , pre+'Z +        // hand-written rule', 1)
        self.g4rule(pre+"ID0", pre+"L | '_'    // hand-written rule", 1)
        self.g4rule(pre+"ID1", pre+"ID0 | " + pre + "N    // hand-written rule", 1)
        self.g4rule('ID' , pre+"ID0 "   + pre + "ID1 *  // hand-written rule", 1)

    def g4hello(self):
        """test_Codepoint.g4
Automatically generated Unicode based hello grammar."""
        with open('test_Codepoint.g4', 'w+') as self.grammar:
            self.g4echo('/** \n' + Codepoint.g4hello.__doc__ + '\n */', 1)
            self.g4echo('grammar      test_Codepoint    ;', 1)
            ...
            if False:
                self.g4rule(pre+"WS1", pre +r"Z | [\t\r\n]", 1)
                self.g4rule("WS", pre+r"WS1 + -> skip", 1)
            else:
    ...
    def test(self, sample=None):
        if self.unittest:
            if sample == None:
                ...
                # Run "make" then "./makeGrammars.py -u" to see results.
